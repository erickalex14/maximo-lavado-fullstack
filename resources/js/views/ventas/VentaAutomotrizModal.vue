        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            >
              ...
              <option
                v-for="producto in productosAutomotrices"
                :key="producto.id"
                :value="producto.id"
              >
...
import type { VentaProductoAutomotriz, CreateVentaAutomotrizRequest, UpdateVentaAutomotrizRequest } from '@/types';

// Props
interface Props {
  visible: boolean;
  mode: 'create' | 'edit' | 'view';
  venta?: VentaProductoAutomotriz;
}
...
const emit = defineEmits<{
  ...
  'saved': [venta: VentaProductoAutomotriz];
}>();
...
const form = ref<CreateVentaAutomotrizRequest>({
  producto_id: 0,
  cliente_id: null,
  cantidad: 1,
  precio_unitario: 0,
...
const modalTitle = computed(() => {
  switch (props.mode) {
    case 'create': return 'Nueva Venta Automotriz';
    case 'edit': return 'Editar Venta Automotriz';
    case 'view': return 'Detalle de Venta Automotriz';
    default: return 'Venta Automotriz';
  }
});

const productosAutomotrices = computed(() => ventaStore.productosAutomotrices);
...
watch(() => form.value.producto_id, (newProductoId) => {
  if (newProductoId && props.mode !== 'view') {
    const producto = productosAutomotrices.value.find(p => p.id === newProductoId);
    if (producto) {
...
function validateForm(): boolean {
  ...
  if (form.value.producto_id && form.value.cantidad > 0) {
    const producto = productosAutomotrices.value.find(p => p.id === form.value.producto_id);
...
async function handleSubmit() {
  ...
  try {
    ...
    let result: VentaProductoAutomotriz | null = null;
    
    if (props.mode === 'create') {
      result = await ventaStore.createVentaAutomotriz(form.value);
    } else if (props.mode === 'edit' && props.venta) {
      const updateData: UpdateVentaAutomotrizRequest = { ...form.value };
      result = await ventaStore.updateVentaAutomotriz(props.venta.id, updateData);
    }
    ...
    console.error('Error al guardar venta automotriz:', error);
...
onMounted(async () => {
  ...
  if (productosAutomotrices.value.length === 0) {
