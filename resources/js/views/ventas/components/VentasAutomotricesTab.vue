<template>
  <div class="ventas-automotrices-tab">
    <!-- Filtros específicos para automotrices -->
    <div class="bg-white dark:bg-surface-800 rounded-lg shadow-sm border border-surface-200 dark:border-surface-700 p-6 mb-6">
      ...
      <div class="flex justify-between items-center mt-4">
        <div class="text-sm text-surface-600 dark:text-surface-400">
          Total: {{ pagination.total }} ventas automotrices
        </div>
            ...
            <tr v-else-if="!loading && filteredVentas.length === 0">
              <td colspan="8" class="px-6 py-8 text-center text-surface-500 dark:text-surface-400">
                <div class="flex flex-col items-center">
                  ...
                  <p class="text-lg font-medium">No hay ventas automotrices</p>
              ...
              <td class="px-6 py-4 whitespace-nowrap">
                <div>
                  <div class="text-sm font-medium text-surface-900 dark:text-surface-100">
                    {{ venta.producto_automotriz?.nombre }}
                  </div>
                  <div v-if="venta.producto_automotriz?.codigo" class="text-sm text-surface-500 dark:text-surface-400">
                    Código: {{ venta.producto_automotriz.codigo }}
                  </div>
                </div>
              </td>
...
import type { VentaProductoAutomotriz } from '@/types';
...
const selectedVenta = ref<VentaProductoAutomotriz | null>(null);

// Computed
const loading = computed(() => ventaStore.loading);
const ventas = computed(() => ventaStore.ventasAutomotrices);
const pagination = computed(() => ventaStore.ventasAutomotricesPagination);
...
const filteredVentas = computed(() => {
  ...
  if (searchTerm.value) {
    ...
    result = result.filter(v => 
      v.producto_automotriz?.nombre?.toLowerCase().includes(term) ||
      v.producto_automotriz?.codigo?.toLowerCase().includes(term)
    );
  }
...
function applyFilters() {
  ...
  ventaStore.fetchVentasAutomotrices(filters);
}
...
async function refreshData() {
  await ventaStore.fetchVentasAutomotrices();
}

function viewVenta(venta: VentaProductoAutomotriz) {
  selectedVenta.value = venta;
  modalMode.value = 'view';
  showModal.value = true;
}

function editVenta(venta: VentaProductoAutomotriz) {
  selectedVenta.value = venta;
  modalMode.value = 'edit';
  showModal.value = true;
}

async function deleteVenta(venta: VentaProductoAutomotriz) {
  if (!confirm(`¿Estás seguro de que deseas eliminar esta venta de ${venta.producto_automotriz?.nombre}?`)) {
    return;
  }
...
function handlePageChange(page: number) {
  ventaStore.fetchVentasAutomotrices({ page });
}

function handlePerPageChange(perPage: number) {
  ventaStore.fetchVentasAutomotrices({ per_page: perPage, page: 1 });
}
