<?php

namespace App\Http\Requests\Egreso;

use Illuminate\Foundation\Http\FormRequest;

class CreateEgresoRequest extends FormRequest
{
    public function authorize(): bool
    {
        return true;
    }

    public function rules(): array
    {
        return [
            'fecha' => 'required|date|before_or_equal:today',
            'tipo' => 'required|in:salario,proveedor,gasto_general',
            'referencia_id' => 'nullable|integer|min:1',
            'monto' => 'required|numeric|min:0|max:999999.99',
            'descripcion' => 'nullable|string|max:255',
        ];
    }

    public function messages(): array
    {
        return [
            'fecha.required' => 'La fecha es obligatoria.',
            'fecha.date' => 'La fecha debe ser válida.',
            'fecha.before_or_equal' => 'La fecha no puede ser futura.',
            'tipo.required' => 'El tipo es obligatorio.',
            'tipo.in' => 'El tipo debe ser: salario, proveedor o gasto_general.',
            'referencia_id.integer' => 'La referencia debe ser un número entero.',
            'referencia_id.min' => 'La referencia debe ser mayor a 0.',
            'monto.required' => 'El monto es obligatorio.',
            'monto.numeric' => 'El monto debe ser un número.',
            'monto.min' => 'El monto debe ser mayor o igual a 0.',
            'monto.max' => 'El monto no puede ser mayor a 999,999.99.',
            'descripcion.string' => 'La descripción debe ser texto.',
            'descripcion.max' => 'La descripción no puede exceder 255 caracteres.',
        ];
    }
}
