...
<body>
    ...
    <div class="header">
        <div class="header-left">
            ...
            <div class="empresa-info">
                <p><strong>RUC:</strong> {{ $factura->ruc_emisor }}</p>
                <p><strong>Dirección:</strong> {{ $factura->direccion_emisor }}</p>
        ...
        <div class="header-right">
            <div class="factura-info">
                ...
                <p style="font-size: 14px; margin: 5px 0;">
                    {{ str_pad($factura->establecimiento, 3, '0', STR_PAD_LEFT) }}-{{ str_pad($factura->punto_emision, 3, '0', STR_PAD_LEFT) }}-{{ str_pad($factura->secuencial, 9, '0', STR_PAD_LEFT) }}
                </p>
                <p class="factura-fecha">
                    Fecha: {{ 
                        is_string($factura->venta->fecha) 
                ...
                <p style="font-size: 9px; margin: 8px 0 0 0;">
                    <span class="estado-badge estado-{{ strtolower($factura->estado_sri) }}">
                        {{ $factura->estado_sri }}
                    </span>
                </p>
            </div>
        ...
        <div class="cliente-info">
            <div class="cliente-left">
                <div class="info-row">
                    ...
                    <span>{{ $factura->razon_social_comprador }}</span>
                </div>
                <div class="info-row">
                    <span class="info-label">Cédula/RUC:</span>
                    <span>{{ $factura->identificacion_comprador }}</span>
                </div>
            </div>
            <div class="cliente-right">
                @if($factura->direccion_comprador)
                    <div class="info-row">
                        ...
                        <span>{{ $factura->direccion_comprador }}</span>
                    </div>
                @endif
                @if($factura->email_comprador)
                    <div class="info-row">
                        <span class="info-label">Email:</span>
                        <span>{{ $factura->email_comprador }}</span>
                    </div>
                @endif
            </div>
            <div style="clear: both;"></div>
    ...
    <table class="detalle-table">
        ...
        <tbody>
            @if($factura->venta->detalles && $factura->venta->detalles->count() > 0)
        ...
        <table>
            ...
            @if($factura->descuento > 0)
                <tr class="subtotal-row">
                    ...
                    <td style="text-align: right; color: #d32f2f;">-${{ number_format($factura->descuento, 2) }}</td>
                </tr>
            @endif
            <tr class="subtotal-row">
    ...
    @if($factura->estado_sri === 'AUTORIZADA')
        <div class="sri-info">
            ...
            <p><strong>Clave de Acceso:</strong> {{ $factura->clave_acceso }}</p>
            <p><strong>Número de Autorización:</strong> {{ $factura->numero_autorizacion }}</p>
            <p><strong>Fecha y Hora de Autorización:</strong> {{ 
                is_string($factura->fecha_autorizacion) 
                ? \Carbon\Carbon::parse($factura->fecha_autorizacion)->format('d/m/Y H:i:s') 
                : $factura->fecha_autorizacion->format('d/m/Y H:i:s') 
            }}</p>
            @if($factura->mensaje_sri)
                <p><strong>Mensaje SRI:</strong> {{ $factura->mensaje_sri }}</p>
            @endif
        </div>
    @elseif($factura->estado_sri === 'RECHAZADA')
        <div class="sri-info" style="background-color: #f8d7da; border-color: #dc3545;">
            ...
            @if($factura->mensaje_sri)
                <p><strong>Motivo:</strong> {{ $factura->mensaje_sri }}</p>
            @endif
    ...
    @else
        <div class="sri-info" style="background-color: #fff3cd; border-color: #ffc107;">
            ...
            <p>Estado actual: <strong>{{ $factura->estado_sri }}</strong></p>
    ...
    <div class="footer">
        ...
        <div class="legal-info">
            ...
            <p><small>Generado el {{ now()->format('d/m/Y H:i:s') }}</small></p>
        </div>
    </div>
</body>
</html>
