      ...
      <tbody class="bg-white divide-y divide-gray-200">
        ...
        >
          ...
          <td class="px-6 py-4">
            ...
            <div class="text-sm text-gray-500 max-w-xs truncate">{{ proveedor.descripcion || 'Sin descripción' }}</div>
          </td>

          <!-- Contacto -->
          <td class="px-6 py-4">
            <div class="text-sm text-gray-900">{{ proveedor.email || 'Sin email' }}</div>
            <div class="text-sm text-gray-500">{{ proveedor.telefono || 'Sin teléfono' }}</div>
          </td>
          ...
          <td class="px-6 py-4 whitespace-nowrap">
            <div class="flex items-center gap-2">
              <span class="text-sm font-medium text-gray-900">
                ${{ proveedor.deuda_pendiente.toLocaleString('es-CO', { minimumFractionDigits: 2 }) }}
              </span>
              <span
                :class="{
                  'bg-green-100 text-green-800': proveedor.deuda_pendiente === 0,
                  'bg-yellow-100 text-yellow-800': proveedor.deuda_pendiente > 0 && proveedor.deuda_pendiente <= 100000,
                  'bg-red-100 text-red-800': proveedor.deuda_pendiente > 100000
                }"
          ...
          <td class="px-6 py-4 whitespace-nowrap">
            <span
              :class="{
                'bg-green-100 text-green-800': proveedor.deuda_pendiente === 0,
                'bg-red-100 text-red-800': proveedor.deuda_pendiente > 0
              }"
              class="inline-flex px-2 py-1 text-xs font-semibold rounded-full"
            >
              {{ proveedor.deuda_pendiente === 0 ? 'Al Día' : 'Con Deuda' }}
            </span>
          </td>
