        ...
        <div>
          ...
          <div class="relative">
            ...
            <input
              ...
              required
              placeholder="0.00"
              class="w-full pl-8 pr-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-transparent"
...
const form = ref({
  ...
  tipo: 'salario' as 'salario' | 'proveedor' | 'gasto_general',
...
// Computed
const isEditing = computed(() => !!props.egreso);
...
const handleSubmit = async () => {
  ...
  try {
    if (isEditing.value && props.egreso) {
      // TODO: Implementar updateEgreso cuando esté disponible
      console.log('Edición de egreso no implementada aún');
      emit('save');
      return;
    } else {
      ...
      await finanzasStore.createEgreso(createRequest as any);
    }
  ...
  if (newValue) {
    if (props.egreso) {
      ...
      form.value = {
        ...
        referencia_id: props.egreso.referencia_id || null,
...
watch(() => props.egreso, (newEgreso) => {
  if (newEgreso && props.isOpen) {
    form.value = {
      fecha: newEgreso.fecha,
      tipo: newEgreso.tipo,
      referencia_id: newEgreso.referencia_id || null,
